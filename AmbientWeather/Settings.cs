...
public class Settings
{
    ...
    public TimeSpan SamplingInterval => TimeSpan.FromMinutes(SamplingIntervalData);

    private readonly byte CurrentUnitSettingFlag1;
    public TemperatureUnit IndoorTemperatureUnit => (TemperatureUnit)(CurrentUnitSettingFlag1 & 0x01);
    public TemperatureUnit OutdoorTemperatureUnit => (TemperatureUnit)((CurrentUnitSettingFlag1 & 0x02) >> 1);
    public RainfallUnits RainfallUnits => (RainfallUnits)((CurrentUnitSettingFlag1 & 0x04) >> 2);
    public PressureUnits PressureUnits => (PressureUnits)((CurrentUnitSettingFlag1 & 0xE0) >> 5);

    private readonly byte CurrentUnitSettingFlag2;
    public WindSpeedUnits WindSpeedUnits => (WindSpeedUnits)(CurrentUnitSettingFlag2 & 0x1F);

    private readonly byte DisplayFormatFlag1;
    public PressureType PressureType => (PressureType)(DisplayFormatFlag1 & 0x01);
    ...
    public short OutdoorTemperatureOffset => OutdoorTemperatureOffsetData.MsbSigned();

    private readonly ushort IndoorTemperatureOffsetData;
    public short IndoorTemperatureOffset => IndoorTemperatureOffsetData.MsbSigned();

    private readonly ushort OutdoorHumidityOffsetData;
    public short OutdoorHumidityOffset => OutdoorHumidityOffsetData.MsbSigned();
